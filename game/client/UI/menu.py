...
class MainWidget(QMainWindow, Ui_MainWindow):
    ...
    def handle_connection(self):
        server_ip, ok_pressed = QInputDialog.getText(
            self, 'Подключение к серверу', 'Введите ip сервера')

        if ok_pressed:
            self.close()

            main(server_ip, 27520)
    ...
    def edit_size(self):
        width, ok_pressed = QInputDialog.getInt(
            self, "Введите ширину", "Введите ширину окна",            1920, 1, 10000, 1)
        if ok_pressed:
            height, ok_pressed = QInputDialog.getInt(
                self, "Введите высоту", "Введите высоту окна",                1080, 1, 7000, 1)
            if ok_pressed:
                self.setGeometry(0, 0, width, height)

        AppState.set_screen_resolution(
